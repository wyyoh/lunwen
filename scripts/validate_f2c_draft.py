...
def validate(root: Path) -> dict:
    ...
    commands = {
        "pytest": [sys.executable, "-m", "pytest", "-q", "-p", "no:cacheprovider", *relative_tests],
        "ruff": [sys.executable, "-m", "ruff", "check", "--no-cache", "--output-format", "concise", *lint_paths],
        "train_smoke": [sys.executable, "-m", "keyed_gram.stage_f2c", "--smoke"],
    }
    env = dict(os.environ)
    env.update({"PYTHONPATH": str(root / "src"), "PYTHONDONTWRITEBYTECODE": "1",
                "PYTEST_DISABLE_PLUGIN_AUTOLOAD": "1", "NO_COLOR": "1", "PYTHONUTF8": "1"})
    results = {}
    for label, command in commands.items():
        ...
        results[label] = {"exit_code": process.returncode, "stdout": process.stdout,
    ...
    return {
        ...
        "unit_validation_status": "passed" if all(r["exit_code"] == 0 for r in results.values()) else "failed",
        "platform": platform.platform(), "python": platform.python_version(),
        "packages": {name: importlib.metadata.version(name)
                     for name in ("z3-solver", "pytest", "ruff", "pyyaml")},
        "test_files": relative_tests, "commands": results, "source_files": records,
        "container_environment_verified": False,
        "full_repository_tests_run": False,
        "formal_development_started": False, "formal_locked_started": False,
        "analyzer_frozen": False, "ready_for_formal_f2c_audit": False,
    }
